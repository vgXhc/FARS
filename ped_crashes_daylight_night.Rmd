...
The United States has a pedestrian fatality crisis.
After years of progress, the number of people killed while walking started rising again around 2010, contrary to what's happening outside of the US.
...
![This chart shows the deadliest time of day for pedestrians, averaging data from 2000 to 2021 over a whole calendar year. Image: New York Times.](img/nyt_chart_deaths_by_time_of_day_and_month.png)

It occurred to me and my friend and fellow data nerd Austin Griessbach that we can explicitly look at sunrise and sunset times and how they correlate with the time of a crash.
This turned out to be a more complicated process than expected and I document my part of it at the end of the article, in the \@ref(methods-and-code).
Most people probably won't care about that part though, and so let's dive into our findings.
...
```{r}
#load cleaned data. Data cleaning is documented in Methods section of this article
...
peds |> 
  ...
  annotate(geom = "text", x = .5, y = 5400, label = "Peak about 20-50\n minutes after sunset", family = "Roboto Condensed" ) +
    annotate("segment", x = 22.5/60, xend = 52.5/60, y = 4950, yend = 4950,
...
peds |> 
  ...
  geom_histogram(binwidth = 1/4) +
    stat_bin(binwidth= 1/4, geom= "text", aes(label=..count..), vjust=-1.5)
  geom_vline(xintercept = 0, color = "red", alpha = 0.3, linewidth = 1) +
  # facet_wrap(~YEAR) +
  ylab("Pedestrian fatalities") +
  xlab("Hours from sunset") +
  ylim(c(0,5500)) +
  labs(title = "Pedestrian fatalities have a less pronounced before sunrise",
       subtitle = "Fatal pedestrian crashes in the US, 2001-2021; 15-minute bins",
       caption = "Data: Fatality Analysis Reporting System (FARS)\nVisualization: Harald Kliems") +
  theme_ipsum(base_family = "Roboto Condensed") +
  annotate(geom = "text", x = -.5, y = 2450, label = "Peak about 20-50\n minutes before sunrise", family = "Roboto Condensed" ) +
    annotate("segment", x = -22.5/60, xend = -52.5/60, y = 1980, yend = 1980,
           arrow = arrow(ends = "both", angle = 90, length = unit(.2,"cm"))) +
...
peds_state_facets|>   
  ...
  facet_wrap(~states_binned, scales = "free_y") +
    ylab("Pedestrian fatalities") +
  xlab("Hours from sunset") +
  labs(title = "Pedestrian fatalities around sunset have a similar pattern across US States",
       subtitle = "Fatal pedestrian crashes in the US, 2001-2021; 15-minute bins",
       caption = "Data: Fatality Analysis Reporting System (FARS)\nVisualization: Harald Kliems") +
  theme_ipsum(base_family = "Roboto Condensed") +
...
peds |> 
  ...
  facet_wrap(~ label_month) +
    ylab("Pedestrian fatalities") +
  xlab("Hours from sunset") +
  labs(title = "Pedestrian fatalities around sunset have a similar pattern throughout the year",
       subtitle = "Fatal pedestrian crashes in the US, 2001-2021; 15-minute bins",
       caption = "Data: Fatality Analysis Reporting System (FARS)\nVisualization: Harald Kliems") +
  theme_ipsum(base_family = "Roboto Condensed") +
  theme(panel.grid.major = element_blank(),
...
table_month |> 
  ...
  labs(fill = "Before/after sunset", title = "Fatal pedestrian crashes before and after sunset",
...
```{r}
peds |> 
  mutate(year_period = case_match(YEAR,
  ...
  facet_wrap(~ year_period) +
      ylab("Pedestrian fatalities") +
  xlab("Hours from sunset") +
  labs(title = "The pattern of pedestrian fatalities around sunset has not changed much from 2001 to 2020",
...
Unfortunately, the crash time is made available only down to the hour, making it impossible to do a 1:1 comparison in relation to the sunrise/sunset times. Additionally, only data from 2016-2022 is available. Here's the hourly distribution fatalities to see how German and US pedestrian fatalities compare.

```{r}
crashes_de <- readRDS("data/crashes_de.RDS")

us_crashes <- peds |> 
  sf::st_drop_geometry() |> 
  mutate(crash_hour = HOUR) |> 
  filter(YEAR > 2015) |> 
  group_by(crash_hour) |> 
  tally() |> 
  mutate(country = "US")
...
de_crashes_fatal <- crashes_de |> 
  ...
  mutate(
    ...
    crash_type = case_when(UKATEGORIE == 1 ~ "fatal",
  ...
  group_by(crash_hour, country) |> 
  tally()



rbind(us_crashes, de_crashes_fatal) |> 
  ...
  labs(title = "The pattern of fatal pedestrian crashes by time of day differs between the US and Germany",
       fill = element_blank(),
       subtitle = "2016-2022 crash data",
       caption = "Data: Fatality Analysis Reporing System (FARS); Unfallatlas <br> Data for Germany is the number of fatal _crashes_ involving a pedestrian; US data is number _fatalities_ for pedestrians<br>Visualization: Harald Kliems",
       x = "Time of day",
...
Not only is the number of pedestrian fatalities in Germany a lot lower than in the US[^1], the distribution across the day also looks quite different.
The first peak is much later in Germany; the second peak is both earlier and narrower, centered around evening rush hour.

[^1]: To put this in perspective: According to Wikipedia, the US population in 2020 was 331 million; Germany's was 83 million. From 2016 to 2022, Germany had `r round(2139/83)` fatal pedestrian crashes per 1 million population; the US had `r round(38397/331)` pedestrian fatalities.


# Conclusion: What is going on??

Austin and I split the data in many other ways to try and elucidate what it going on, and to confirm that we weren't looking at weird artefacts or straight out errors in our code or in the data. At this point were certain enough that the data are correct to publish this post. If you find any errors in the code, please let us know!

As _why_ were seeing what we're seeing: We're still stumped. Why such a rapid increase after (but not before!) sunset? Why is the pattern different in the US than in Germany? We can't say. Additional data would be needed to make progress on an explanation. 
...
It may also help to look at other crash types, such as fatal crashes _not_ involving pedestrians; fatal bike crashes; or non-fatal injury crashes.
The FARS data only contains fatalities, and while some states make non-fatal crash data available, there are issues with the comparability of the data over time and between states. And a lot of data cleaning.

For now, we'll leave it to the readers to come up with explanations, based on data or speculation. And maybe publish a follow-up post at a later point.

## Methods
The following contains and describes the code needed to prepare the data for the plots and tables above. We want our data to be reproducible and open to checking by others. 
...
The structure of the folders and files also has some slight variations between years and therefore reading in the files also requires some effort and multiple functions.
...
Additionally, the variables and format for the latitude and longitude are also inconsistent between years.
...
I don't fully understand what type of fatalities those are and will exclude them from analysis.
...
# Connecting crashes to census data
...
```{r}
get_block_groups <- function(state, year){
  block_groups <- block_groups(state = state, year = year, cb = TRUE)
}

states <- unique(peds$STATE)


states_years <- expand_grid(state = states, year = 2013:2014)

# states_years |> 
#   rowwise() |> 
#   map2_dfr(~ get_block_groups(state = states_years$state, year = states_years$year))

states_geo_13_20 <- pmap(states_years, get_block_groups)

```

# German crashes

Available from <https://unfallatlas.statistikportal.de/>

Documentation: <https://www.opengeodata.nrw.de/produkte/transport_verkehr/unfallatlas/DSB_Unfallatlas_EN.pdf>

Coordinates are in `LINREFX` and `LINREFY`
...
crashes_de |> 
  ...
  mutate(
    ...
    crash_type = case_when(UKATEGORIE == 1 ~ "fatal",
                                ...
                                UKATEGORIE == 3 ~ "minor injury"),
    crash_type = factor(crash_type, levels = c("fatal", "serious injury", "minor injury"), ordered = TRUE)) |> 
  group_by(crash_hour, crash_type) |> 
  tally() |> 
  ggplot(aes(crash_hour, n)) +
  geom_col() +
  facet_wrap(~ crash_type, scales = "free_y") +
  hrbrthemes::theme_ipsum_rc() +
  labs(title = "Crashes involving pedestrians by time of crash and crash severity",
       subtitle = "Crashes in Germany, 2016-2022",
       caption = "Data: https://unfallatlas.statistikportal.de/\nVisualization: Harald Kliems",
       x = "Time of day",
       y = "Number of crashes") +
  scale_x_continuous(breaks = c(0, 6, 12, 18))
...
rbind(us_crashes, de_crashes_fatal) |> 
  ggplot(aes(crash_hour, country, fill = country)) +
  ggridges::geom_density_ridges(rel_min_height = 0.01, scale = 8, alpha = 0.3) +
  theme_minimal() +
  xlim(c(0, 24))
```

Density plots would be nice for the data, but they are not straighforward for circular, 24h data.
...
circ_data <- rbind(us_crashes, de_crashes_fatal) |> 
  mutate(half_hour = paste0(crash_hour,":30"),
         half_hour_time = hm(half_hour),
         times_in_decimal = crash_hour/60,
